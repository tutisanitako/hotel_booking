                            ...
                            <td>
                                <div class="btn-group">
                                    <button class="btn btn-sm btn-warning" 
                                            onclick="editRoom({{ $room->id }}, '{{ $room->name }}', '{{ $room->type }}', '{{ $room->description }}', {{ $room->price_per_night }}, {{ $room->max_adults }}, {{ $room->max_children }}, {{ $room->available_rooms }}, {{ $room->size_sqm ?? 0 }}, [{{ $room->amenities->pluck('id')->implode(',') }}])">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <form action="{{ route('admin.hotels.rooms.destroy', [$hotel, $room]) }}" 
                                          method="POST" 
            ...
            <form id="editRoomForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Available Rooms *</label>
                            <input type="number" name="available_rooms" id="edit_available" class="form-control" min="1" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Size (m²)</label>
...
function editRoom(id, name, type, description, price, adults, children, available, size, amenities) {
    ...
    document.getElementById('editRoomForm').action = 
        '{{ route("admin.hotels.rooms.update", [$hotel, ":id"]) }}'.replace(':id', id);
